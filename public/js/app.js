...
async function addTask(title, department, priority, notes, source, attachments, dueDate, recurring, assignedTo, subDepartment, startDate) {
  const taskData = {
    title: title.trim(),
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    <div class="detail-grid">
      ...
      <div>
        ...
        ${task.startDate && task.dueDate ? `<span>${formatDueDate(task.dueDate, task.status === 'Completed', task.startDate)}</span>` : task.dueDate ? `<span>${formatDueDate(task.dueDate, task.status === 'Completed')} ${new Date(task.dueDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>` : '<span style="color:var(--color-text-light);font-size:0.85rem;">Not set</span>'}
      </div>
      <div>
...
async function init() {
  ...
  document.getElementById('btn-create-parsed-task').addEventListener('click', async () => {
    ...
    await addTask(title, dept, priority, notes, 'manual', [], dueDate, recurring, assignedTo, subDept, startDate);
  ...
  document.getElementById('form-add-task').addEventListener('submit', async (e) => {
    ...
    if (editingTaskId) {
      const updates = { title, department, subDepartment, priority, notes, startDate, dueDate, recurring, attachments: allAttachments };
      if (assignTo) {
      ...
      await addTask(title, department, priority, notes, 'manual', allAttachments, dueDate, recurring, assignTo, subDepartment, startDate);
    }
